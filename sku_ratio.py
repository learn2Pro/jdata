...
def handle_user_action(user_action):
    ...
    user_action[['user_id', 'type', 'cate', 'brand']] += "|"
    user_action = user_action.groupby(['sku_id']).sum().reset_index()
    return user_action


def get_basic_features(user, type, cate, brand):
    try:
        user = user.split('|')[:-1]
        type = type.split('|')[:-1]
        cate = cate.split('|')[:-1]
        ...
        basic_features += [(0.0 if len_follow == 0 else (float(len_buy) / float(len_follow))),
                           ...
                           len_buy, len(cate), len(user), len(brand)]
...
user_sku_action.loc[:, 'basic_feature'] = user_sku_action.apply(
    lambda x: get_basic_features(x['user_id'], x['type'], x['cate'], x['brand']),
    axis=1)
user_sku_action.loc[:, 'sku_buy_follow_ratio'] = user_sku_action['basic_feature'].apply(lambda x: x[0])
user_sku_action.loc[:, 'sku_buy_cart_ratio'] = user_sku_action['basic_feature'].apply(lambda x: x[1])
user_sku_action.loc[:, 'sku_buy_cancel_ratio'] = user_sku_action['basic_feature'].apply(lambda x: x[2])
user_sku_action.loc[:, 'sku_buy_click_ratio'] = user_sku_action['basic_feature'].apply(lambda x: x[3])
user_sku_action.loc[:, 'sku_buy_view_ratio'] = user_sku_action['basic_feature'].apply(lambda x: x[4])
user_sku_action.loc[:, 'sku_len_buy'] = user_sku_action['basic_feature'].apply(lambda x: x[5])
user_sku_action.loc[:, 'sku_len_cate'] = user_sku_action['basic_feature'].apply(lambda x: x[6])
user_sku_action.loc[:, 'sku_len_user'] = user_sku_action['basic_feature'].apply(lambda x: x[7])
user_sku_action.loc[:, 'sku_len_brand'] = user_sku_action['basic_feature'].apply(lambda x: x[8])
