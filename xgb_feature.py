...
def get_accumulate_action_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        model_df = pd.get_dummies(actions['model_id'], prefix='model')
        actions = pd.concat([actions, type_df, model_df], axis=1)  # type: pd.DataFrame
        # 近期行为按时间衰减
...
def make_test_set(train_start_date, train_end_date):
    ...
    if os.path.exists(dump_path):
        ...
        for i in (1, 2, 3, 5, 7, 10, 15, 21, 30):
            start_days = datetime.strptime(train_end_date, '%Y-%m-%d') - datetime.timedelta(days=i)
